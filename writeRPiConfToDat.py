# python writeRPiConfToDat.py DEFCONF_RP.TXT
...
def GetConf(infn):
    if checkDEFCONF(infn) == False:
        raise ValueError("No DEFCONF file located.")

    else:
        with open(infn) as RPC:
            lines = RPC.readlines()
            ...
            for line in lines:
                if (line != '\n' and  line[0] != '#'):
                    ...
                    elif call == 'TempCheckPeriod':
                        if 0 <= int(value) <= 30:
                            BitArray['TempCheckPeriod'] = int(value)
                        else:
